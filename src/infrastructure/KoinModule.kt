...
import com.hungry.oauthsample.infrastructure.sql.SqlRevokedTokenCache
...
val userKoinModule = module(createdAtStart = true) {
    ...
    single { SqlRevokedTokenCache() as RevokedTokenCache }
    single { SqlClientRepository() as ClientRepository }
    single { SqlCodeRepository() as CodeRepository }
    
    single { PasswordService() }
    single { TokenService(get(), get(), get<AppConfig>().keysConfig) }
